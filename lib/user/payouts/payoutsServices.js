...
async function getPayoutData(details) {
 ...
 async function sendPaymentRequest(details) {
    if(details)
    {
      ...
      if(details?.amount<=100) return mapper.responseMappingWithData(usrConst.CODE.BadRequest, usrConst.MESSAGE.InvalidDetails,"Amount should be greater than 100")
          ...
          return dao.getUserDetails(query).then(async (userData) => {
            ...
            // if (Number(balance) > Number(details.amount)) {
              ...
              // if (gateway == "pinwallet") {
                ...
                const txId = Math.floor(Math.random() * 900000000000000) + 10000000000;
