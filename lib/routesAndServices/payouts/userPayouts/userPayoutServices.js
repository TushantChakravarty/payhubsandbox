...
async function updatePayoutBalance(details) {
  ...
  return validateRequest(details).then(async (response) => {
    if (response == true) {
      ...
      const data ={
        merchantId:user._id,
        merchantEmailId:user.emailId,
        merchantName:user.business_name,
        grossAmount:details?.payoutBalance,
        deductedFees:0,
        deductedFeesPercentage: 0,
        currencyRate: 0,
        currencyNetCharge: 0,
        payoutBalance:details?.payoutBalance,
        remark:"test",
        transaction_date:today.toISOString(),
      }
  
    
        const updated = await  TopupTransactions.create(data)
